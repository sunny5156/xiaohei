...
from client import xiaoheipath
from client import diagnose
from client import WechatBot
from client.conversation import Conversation

# Add xiaoheipath.LIB_PATH to sys.path
sys.path.append(xiaoheipath.LIB_PATH)

parser = argparse.ArgumentParser(description='XiaoHei Voice Control Center')
...
parser.add_argument('--verbose', action='store_true', help='Directly print logs rather than writing to log file')
...
class Xioahei(object):
    def __init__(self):
        ...
        if not os.path.exists(xiaoheipath.CONFIG_PATH):
            try:
                os.makedirs(xiaoheipath.CONFIG_PATH)
            except OSError:
                self._logger.error("Could not create config dir: '%s'",xiaoheipath.CONFIG_PATH, exc_info=True)
                raise

        # Check if config dir is writable
        if not os.access(xiaoheipath.CONFIG_PATH, os.W_OK):
            self._logger.critical("Config dir %s is not writable. XiaoHei won't work correctly.",xiaoheipath.CONFIG_PATH)

        config_file = xiaoheipath.config('profile.yml')
        # Read config
...
*******************************************************"
*                 小黑 - 对话机器人                       *
*      (c) 2018 sunny5156 <137898350@qq.com>           *
*       https://github.com/sunny5156/xiaohei           *
*******************************************************

如需查看log，可以执行 `tail -f 小黑所在目录/temp/xiaohei.log`
    ...
    if args.verbose:
        ...
        logging.basicConfig(
            filename=os.path.join(
                xiaoheipath.TEMP_PATH, "xiaohei.log"
            ),
            filemode="w",
            format='%(asctime)s %(filename)s[line:%(lineno)d] \
            %(levelname)s %(message)s',
