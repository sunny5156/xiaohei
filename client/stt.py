...
class PocketSphinxSTT(AbstractSTTEngine):
    ...
    def get_config(cls):
        ...
        profile_path = xiaoheipath.config('profile.yml')
...
class BaiduSTT(AbstractSTTEngine):
    ...
    def get_token(self):
        cache = open(os.path.join(xiaoheipath.TEMP_PATH, 'baidustt.ini'),
                     'a+')
        try:
            pms = cache.readlines()
            if len(pms) > 0:
...
class SnowboySTT(AbstractSTTEngine):
    ...
    def __init__(self, sensitivity, model, hotword):
        ...
        self.model = model
        self.resource_file = os.path.join(xiaoheipath.LIB_PATH,
                                          'snowboy/common.res')
        try:
                ...
                if 'snowboy' in profile:
                    if 'model' in profile['snowboy']:
                        ...
                        config['model'] = os.path.join(
                            xiaoheipath.LIB_PATH, 'snowboy/dingdang.pmdl')
